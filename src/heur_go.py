from heur_aux import is_integer, Mutation
from heur import Heuristic, StopCriterion
from objfun import ObjFun
import numpy as np
import numpy.typing as npt
from typing import Any, Dict, Tuple


class Crossover:
    """
    Baseline crossover - randomly chooses "genes" from parents
    """

    def __init__(self) -> None:
        pass

    def crossover(self, x: np.ndarray, y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
        z = np.array([x[i] if rng.uniform() < 0.5 else y[i] for i in np.arange(x.size)], dtype=x.dtype)
        return z


class UniformMultipoint(Crossover):
    """
    Uniform n-point crossover
    """

    def __init__(self, n: int) -> None:
        super().__init__()
        self.n = n  # number of crossover points

    def crossover(self, x: np.ndarray, y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
        co_n = self.n + 1
        n = np.size(x)
        z = np.zeros_like(x)
        k = 0
        p = np.ceil(n / co_n).astype(int)
        for i in np.arange(1, co_n + 1):
            ix_from = k
            ix_to = np.minimum(k + p, n)
            z[ix_from:ix_to] = x[ix_from:ix_to] if np.mod(i, 2) == 1 else y[ix_from:ix_to]
            k += p
        return z


class RandomCombination(Crossover):
    """
    Randomly combines parents
    """

    def __init__(self) -> None:
        super().__init__()

    def crossover(self, x: np.ndarray, y: np.ndarray, rng: np.random.Generator) -> np.ndarray:
        if is_integer(x):
            z = np.array([rng.integers(np.min([x[i], y[i]]), np.max([x[i], y[i]]) + 1) for i in np.arange(x.size)],
                         dtype=x.dtype)
        else:
            z = np.array([rng.uniform(np.min([x[i], y[i]]), np.max([x[i], y[i]])) for i in np.arange(x.size)],
                         dtype=x.dtype)
        return z


class GeneticOptimization(Heuristic):

    def __init__(self, of: ObjFun, maxeval: int, N: int, M: int, Tsel1: float, Tsel2: float,
                 mutation: Mutation, crossover: Crossover, seed: int = None) -> None:
        Heuristic.__init__(self, of, maxeval, seed)

        assert M > N, 'M should be larger than N'
        self.N = N  # population size
        self.M = M  # working population size
        self.Tsel1 = Tsel1  # first selection temperature
        self.Tsel2 = Tsel2  # second selection temperature
        self.mutation = mutation
        self.crossover = crossover

    @staticmethod
    def sort_pop(pop_x: np.ndarray, pop_f: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ixs = np.argsort(pop_f)
        pop_x = pop_x[ixs]
        pop_f = pop_f[ixs]
        return [pop_x, pop_f]

    @staticmethod
    def rank_select(temp: float, n_max: int, rng: np.random.Generator) -> int:
        u = rng.uniform(low=0.0, high=1.0)
        ix = int(np.minimum(np.ceil(-temp * np.log(u)), n_max)) - 1
        return ix

    def search(self) -> Dict[str, Any]:
        try:
            # Initialization:
            pop_X = np.zeros([self.N, np.size(self.of.a)], dtype=self.of.a.dtype)  # population solution vectors
            pop_f = np.zeros(self.N)  # population fitness (objective) function values
            # a.) generate the population
            for i in np.arange(self.N):
                x = self.of.generate_point(self.rng)
                pop_X[i, :] = x
                pop_f[i] = self.evaluate(x)

            # b.) sort according to fitness function
            [pop_X, pop_f] = self.sort_pop(pop_X, pop_f)

            # Evolution iteration
            while True:
                # 1.) generate the working population
                work_pop_X = np.zeros([self.M, np.size(self.of.a)], dtype=self.of.a.dtype)
                work_pop_f = np.zeros(self.M)
                for i in np.arange(self.M):
                    parent_a_ix = self.rank_select(temp=self.Tsel1, n_max=self.N, rng=self.rng)
                    parent_b_ix = self.rank_select(temp=self.Tsel1, n_max=self.N, rng=self.rng)
                    par_a = pop_X[parent_a_ix, :]
                    par_b = pop_X[parent_b_ix, :]
                    z = self.crossover.crossover(par_a, par_b, self.rng)
                    z_mut = self.mutation.mutate(z, self.rng)
                    work_pop_X[i, :] = z_mut
                    work_pop_f[i] = self.evaluate(z_mut)

                # 2.) sort working population according to fitness function
                [work_pop_X, work_pop_f] = self.sort_pop(work_pop_X, work_pop_f)

                # 3.) select the new population (without replacement)
                ixs_not_selected = np.ones(self.M, dtype=bool)
                for i in np.arange(self.N):
                    sel_ix = self.rank_select(temp=self.Tsel2, n_max=np.sum(ixs_not_selected), rng=self.rng)
                    # sel_ix is relative to the filtered subset — resolve to absolute index
                    abs_ix = int(np.where(ixs_not_selected)[0][sel_ix])
                    pop_X[i, :] = work_pop_X[abs_ix, :]
                    pop_f[i] = work_pop_f[abs_ix]
                    ixs_not_selected[abs_ix] = False

                # 4.) sort according to fitness function
                [pop_X, pop_f] = self.sort_pop(pop_X, pop_f)

        except StopCriterion:
            return self.report_end()
